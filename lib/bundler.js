...
function addScriptsToBundle(name, list, bundler) {
  ...
  for (var ix in list) {
    ...
    var fileid = 'loopback-bootnext#' + name + '#' + path.relative(root, filepath);
...
function bundleInstructions(instructions, bundler) {
   ...
   * https://github.com/substack/node-browserify/issues/771
   ...
   b.require(instructionsStream, { expose: 'loopback-bootnext#instructions' });
   */
  ...
  var moduleName = 'loopback-bootnext#' + instructionId;
